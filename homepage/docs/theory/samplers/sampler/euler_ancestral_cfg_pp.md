...
family: Eulerstochastic: yescfg_pp: yesgpu_variant: nostandalone: no
**ComfyUI 함수 시그니처**
`sample_euler_ancestral_cfg_pp(model, x, sigmas, extra_args=None, callback=None, disable=None, eta=1., s_noise=1., noise_sampler=None)`

**docstring:** Ancestral sampling with Euler method steps (CFG++).
$$ x_{k+1}=x_k+d_k(\sigma_{\mathrm{down}}-\sigma_k)+s_{\mathrm{noise}}\sigma_{\mathrm{up}}\xi_k $$
...
$$\lambda=\log\alpha-\log\sigma,\quad x_{k+1}=A_kx_k+B_k\hat{x}_{0,k}+C_k(\text{history})+D_k\xi_k$$$$v_{\mathrm{cfg}}=v_u+w(v_c-v_u),\quad v_{\mathrm{cfg++}}=\Pi_{\mathcal{T}_{\rho}}(v_{\mathrm{cfg}})$$$$x_{k+1}=m_k(x_k)+G_k\xi_k,\ \xi_k\sim\mathcal{N}(0,I),\ \mathrm{Cov}[x_{k+1}|x_k]=G_kG_k^\top$$
**family:** Euler / **stochastic:** yes

## 유도 스케치(순수수학) / 구현 절차(수치해석)

**대상:** `euler_ancestral_cfg_pp` / **family:** Euler
...
**ComfyUI 경로:** `comfy/k_diffusion/sampling.py::sample_euler_ancestral_cfg_pp`
