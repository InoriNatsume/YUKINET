...
**ComfyUI 함수 시그니처**
`sample_dpmpp_sde(model, x, sigmas, extra_args=None, callback=None, disable=None, eta=1., s_noise=1., noise_sampler=None, r=1 / 2)`

**docstring:** DPM-Solver++ (stochastic).
$$ x_{k+1}= \Phi_{\mathrm{drift}}(r,\lambda_s,\lambda_t)+\Phi_{\mathrm{noise}}(\eta,s_{\mathrm{noise}}) $$
DPM++ 계열 stochastic 해석.
...
**ComfyUI 경로:** `comfy/k_diffusion/sampling.py::sample_dpmpp_sde`
...
| r | 1 / 2 | 중간 stage 비율 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
...
| r | $r\in(0,1)$ | 중간 stage 위치 | 2-stage 비율 |
