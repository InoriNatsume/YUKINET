...
1. max_stage만큼 stage를 누적해 drift/noise를 합성한다.
2. noise_scaler, s_noise로 확률항 크기를 제어한다.
3. 구조 보존과 질감 다양성은 stage 수와 noise 스케일의 trade-off다.

**ComfyUI 경로:** `comfy/k_diffusion/sampling.py::sample_er_sde`
...
| noise_scaler | None | ER-SDE 노이즈 스케일 함수 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
| max_stage | 3 | ER-SDE stage 수 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
...
| noise_scaler | $c_g$ | diffusion 계수 | ER-SDE 노이즈 스케일 |
| max_stage | $M$ | stage 수 | 다단계 SDE stage 상한 |
