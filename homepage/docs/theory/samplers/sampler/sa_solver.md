...
# Sampler: sa_solver
family: SA-Solverstochastic: yescfg_pp: nogpu_variant: nostandalone: no
**ComfyUI 함수 시그니처**
`sample_sa_solver(model, x, sigmas, extra_args=None, callback=None, disable=False, tau_func=None, s_noise=1.0, noise_sampler=None, predictor_order=3, corrector_order=4, use_pece=False, simple_order_2=False)`

**docstring:** Stochastic Adams Solver with predictor-corrector method (NeurIPS 2023).
...
| 모델 평가량(경향) | stage 기반 predictor/corrector 조합으로 평가 횟수가 증가할 수 있다. |
...
**family:** SA-Solver / **stochastic:** yes

## 유도 스케치(순수수학) / 구현 절차(수치해석)

**대상:** `sa_solver` / **family:** SA-Solver
...
**ComfyUI 경로:** `comfy/k_diffusion/sampling.py::sample_sa_solver`
...
| 간격 함수 | $\tau:[0,1]\to[0,1]$ | 측정가능성/유계성 가정. |
...
|---|---|---|---|
| tau_func | None | SA-Solver stochastic interval 함수 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
| s_noise | 1.0 | noise term 배수 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
| noise_sampler | None | 코드 레벨 노이즈 샘플러 함수 주입 | 코드 레벨 파라미터. |
| predictor_order | 3 | SA-Solver predictor 차수 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
| corrector_order | 4 | SA-Solver corrector 차수 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
| use_pece | False | PECE 모드 사용 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
| simple_order_2 | False | 저차 안정화 옵션 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
...
| tau_func | $\tau(t)$ | stochastic interval | 노이즈 적용 구간 함수 |
...
| predictor_order | $p$ | predictor 차수 | 예측 단계 차수 |
| corrector_order | $q$ | corrector 차수 | 보정 단계 차수 |
| use_pece | $\mathbf{1}_{\mathrm{PECE}}$ | PECE 토글 | predict-evaluate-correct-evaluate |
| simple_order_2 | $\mathbf{1}_{2nd}$ | 2차 단순화 | order-2 단순 모드 |
