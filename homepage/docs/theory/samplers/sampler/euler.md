...
# Sampler: euler
family: Eulerstochastic: nocfg_pp: nogpu_variant: nostandalone: yes
**ComfyUI 함수 시그니처**
`sample_euler(model, x, sigmas, extra_args=None, callback=None, disable=None, s_churn=0., s_tmin=0., s_tmax=float('inf'), s_noise=1.)`

**docstring:** Implements Algorithm 2 (Euler steps) from Karras et al. (2022).
$$ x_{k+1}=x_k+d_k(\sigma_{k+1}-\hat{\sigma}_k),\ d_k=\frac{x_k-\hat{x}_{0,k}}{\hat{\sigma}_k} $$
...
**ComfyUI 경로:** `comfy/k_diffusion/sampling.py::sample_euler`
