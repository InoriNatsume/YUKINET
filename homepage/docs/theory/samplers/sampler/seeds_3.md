...
# Sampler: seeds_3
**ComfyUI 함수 시그니처**
`sample_seeds_3(model, x, sigmas, extra_args=None, callback=None, disable=None, eta=1., s_noise=1., noise_sampler=None, r_1=1./3, r_2=2./3)`

**docstring:** SEEDS-3 - Stochastic Explicit Exponential Derivative-free Solvers (VP Data Prediction) stage 3.
...
3-stage/3M 계열로 higher-order correction을 사용한다. 낮은 step에서도 구조 보존성이 좋지만 구현/튜닝 민감도가 높다.
...
| 항목 | 내용 |
|---|---|
| method class | 3-stage/3M high-order |
| local truncation | $O(h^4)\ \text{(근사적)}$ |
| global error | $O(h^3)\ \text{(근사적)}$ |
| strong/weak 관점 | 고차 correction으로 drift 편향은 낮지만 step/스케줄 민감도가 높음 |
| stability 메모 | 충분한 step에서 구조 보존 우수, 거친 설정에서는 진동 가능성 |
...
| history 버퍼 | 다중 stage history(2~3단계 수준)를 유지한다. |
...
**ComfyUI 경로:** `comfy/k_diffusion/sampling.py::sample_seeds_3`
...
| 중간 stage1 | $0<r_1<1$ | SEEDS-3 첫 중간점. |
| 중간 stage2 | $0<r_2<1$ | SEEDS-3 둘째 중간점. |
...
| r_1 | 1./3 | SEEDS-3 중간 stage1 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
| r_2 | 2./3 | SEEDS-3 중간 stage2 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
...
| r_1 | $r_1$ | 중간 stage1 | SEEDS-3 첫 stage |
| r_2 | $r_2$ | 중간 stage2 | SEEDS-3 둘째 stage |
