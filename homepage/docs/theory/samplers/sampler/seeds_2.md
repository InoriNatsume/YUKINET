...
1. 중간 sigma를 생성해 2-stage 적분을 수행한다.
2. r과 solver_type이 품질/안정성 균형을 좌우한다.
3. eta, s_noise는 다양성-구조 보존 균형 파라미터다.

**ComfyUI 경로:** `comfy/k_diffusion/sampling.py::sample_seeds_2`
...
| r | 0.5 | 중간 stage 비율 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
| solver_type | "phi_1" | 보정 방식(midpoint/heun 등) 선택 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
