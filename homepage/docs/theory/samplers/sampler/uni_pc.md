...
**ComfyUI 함수 시그니처**
`sample_unipc(model, noise, sigmas, extra_args=None, callback=None, disable=False, variant='bh1')`
...
| 모델 평가량(경향) | 구현 경로에 따라 1~2회 이상 변동 가능. |
...
**ComfyUI 경로:** `comfy/extra_samplers/uni_pc.py::sample_unipc`

**독립 구현 전략:** 현재는 comfy_native 위임 권장(후속 standalone 확장)

**참고:** k_diffusion 경로가 아니라 comfy.extra_samplers.uni_pc 경로
...
|---|---|---|---|
| variant | 'bh1' | sampler-specific control parameter | 맥락 의존. |
...
| noise | $x_0 \text{ or } \xi_0$ | 초기 조건 | 초기 노이즈/초기 latent |
...
| variant | $variant$ | 구현 의존 | sampler-specific tuning parameter |
