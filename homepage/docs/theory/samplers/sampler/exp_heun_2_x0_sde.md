...
family: Heun/ExpHeunstochastic: yescfg_pp: nogpu_variant: nostandalone: no
**ComfyUI 함수 시그니처**
`sample_exp_heun_2_x0_sde(model, x, sigmas, extra_args=None, callback=None, disable=None, eta=1., s_noise=1., noise_sampler=None, solver_type="phi_2")`

**docstring:** Stochastic exponential Heun second order method in data prediction (x0) and logSNR time.
$$ x_{k+1}=\Phi(x_k,\sigma_k,\sigma_{k+1}) $$
Heun/ExpHeun 계열 stochastic 해석.
...
**family:** Heun/ExpHeun / **stochastic:** yes

## 유도 스케치(순수수학) / 구현 절차(수치해석)

**대상:** `exp_heun_2_x0_sde` / **family:** Heun/ExpHeun
...
**ComfyUI 경로:** `comfy/k_diffusion/sampling.py::sample_exp_heun_2_x0_sde`
...
| solver_type | "phi_2" | 보정 방식(midpoint/heun 등) 선택 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
