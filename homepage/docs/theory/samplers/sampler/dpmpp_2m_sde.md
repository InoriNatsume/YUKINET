...
1. 직전 denoised 결과를 버퍼링해 2M correction을 계산한다.
2. solver_type(midpoint/heun)에 따라 보정 계수를 분기한다.
3. expm1 기반 계산으로 작은 step에서 수치 취소오차를 줄인다.

**ComfyUI 경로:** `comfy/k_diffusion/sampling.py::sample_dpmpp_2m_sde`
