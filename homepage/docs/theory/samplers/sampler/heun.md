...
1. step당 모델 평가 2회(예측/보정)를 수행한다.
2. s_churn 활성 시 sigma를 일시적으로 inflate 후 동일 보정 절차를 적용한다.
3. 고정 step 환경에서 안정성은 좋지만 계산량은 Euler 대비 증가한다.

**ComfyUI 경로:** `comfy/k_diffusion/sampling.py::sample_heun`
