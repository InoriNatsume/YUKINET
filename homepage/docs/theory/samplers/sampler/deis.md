...
# Sampler: deis
family: N-DM / DEISstochastic: nocfg_pp: nogpu_variant: nostandalone: no
**ComfyUI 함수 시그니처**
`sample_deis(model, x, sigmas, extra_args=None, callback=None, disable=None, max_order=3, deis_mode='tab')`
...
**family:** N-DM / DEIS / **stochastic:** no

## 유도 스케치(순수수학) / 구현 절차(수치해석)

**대상:** `deis` / **family:** N-DM / DEIS
...
**ComfyUI 경로:** `comfy/k_diffusion/sampling.py::sample_deis`
...
|---|---|---|---|
| max_order | 3 | 최대 이력 차수 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
| deis_mode | 'tab' | DEIS 계수 모드 | Basic KSampler 미노출. Custom Sampling 노드/코드(extra_options)에서 제어. |
...
| max_order | $m_{\max}$ | 최대 차수 | 적응형/다단계 상한 |
| deis_mode | $\mathcal{M}_{\mathrm{DEIS}}$ | 계수 선택 | DEIS coefficient 모드 |
